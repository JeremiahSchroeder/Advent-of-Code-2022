...
def solution():
    ...
    tailPositions=[[0,0]]
    ropeNodes = [[0,0] for _ in range(10)]
    ...
    for move in inputStringList:
        ...
        for i in range(0,numMoves):
            ...
            updateRopeNodes(ropeNodes,direction)
            if not ropeNodes[-1] in tailPositions:
                # print(currentTailPosition)
                tailPositions.append(ropeNodes[-1])
...
def updateHeadPosition(currentHeadPosition, direction):
    if direction == 'U':
...
def updateTailPosition(currentHeadPosition,currentTailPosition):
    ...
    currentTailPosition = list(numpy.add(currentTailPosition, move))
    return currentTailPosition

def updateRopeNodes(ropeNodes,direction):
    updateHeadPosition(ropeNodes[0],direction)
    for i in range(1,len(ropeNodes)):
        ropeNodes[i] = updateTailPosition(ropeNodes[i-1],ropeNodes[i])
